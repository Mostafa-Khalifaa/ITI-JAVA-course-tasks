...
public class LibraryManagementSystem {
    ...
    // Get valid menu choice within range
    ...
    private static void manageItems() {
        ...
        while (!back) {
            ...
            int choice = getValidMenuChoice(1, 10);
    ...
    private static void addMagazine() {
        ...
        // Check if ID already exists
        if (library.idExists(id)) {
            System.out.println("Error: A magazine with ID '" + id + "' already exists!");
            return;
        }
    ...
    private static void deleteMagazine() throws ItemNotFoundException {
        ...
        System.out.print("Are you sure you want to delete this magazine? (yes/no): ");
        String confirm = scanner.nextLine().trim().toLowerCase();
    ...
    private static void addClient() {
        ...
        // Check if ID already exists
        if (clientManager.idExists(id)) {
            System.out.println("Error: A client with ID '" + id + "' already exists!");
            return;
        }
        ...
        if (!email.contains("@") || !email.contains(".")) {
            System.out.println("Warning: Email format might be invalid, but will be saved.");
        }
    ...
    private static void deleteClient() throws ItemNotFoundException {
        ...
        if (!client.getBorrowedItems().isEmpty()) {
            System.out.println("Error: Client has " + client.getBorrowedItems().size() + 
                             " borrowed items.");
            System.out.println("Please return all items before deleting the client.");
            return;
        }
    ...
    private static void borrowItem() throws ItemNotFoundException {
        ...
        if (!clientManager.hasClients()) {
            System.out.println("Error: No clients registered. Please add a client first.");
            return;
        }
        
        // Check if there are any available items
        if (!library.hasAvailableItems()) {
            System.out.println("Error: No items available for borrowing.");
            return;
        }
    ...
    private static void returnItem() throws ItemNotFoundException {
        ...
        // Check if there are any clients
        if (!clientManager.hasClients()) {
            System.out.println("Error: No clients registered.");
            return;
        }
        ...
        if (client.getBorrowedItems().isEmpty()) {
            System.out.println("Error: This client has no borrowed items to return.");
            return;
        }
    ...
    private static void searchItems() {
        if (!library.hasItems()) {
            System.out.println("No items in the library to search.");
            return;
        }
        
        String keyword = getValidInput("Enter search keyword for title: ");
        library.searchByTitle(keyword);
    }
    
    private static void searchClients() {
        if (!clientManager.hasClients()) {
            System.out.println("No clients registered to search.");
            return;
        }
        
        String keyword = getValidInput("Enter search keyword for name: ");
        clientManager.searchByName(keyword);
    }
}